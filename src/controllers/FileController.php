...
class FileController extends ControllerCommon
{
   ...
   public function actionMultiple()
   {
    ...
    public function actionSend()
    {
        ...
        $uploadForm->extensions = $this->request->post()['UploadForm']['extensions'];

        if ($this->request->isPost && $uploadForm->load($this->request->post())) {

            $post = $this->request->post()['UploadForm'];

            $model = $post['model'] ?? null;
            $model_field = $post['model_field'] ?? null;
            $model_field_id = $post['model_field_id'] ?? null;
            $model_id = $post['model_id'] ?? null;

            $file_name = $post['file_name'] ?? false;
            $thumb = $post['thumb'] ?? false;
            $save_file_model = $post['save_file_model'] ?? 0;
            ...
            $fileUploaded = $uploadForm->uploadFile(
                [
                    'file_name'=>$file_name,
                    'thumb'=>$thumb,
                    'folderByType'=>true,
                    'extensions'=>$extensions,
                    'save_file_model'=>$save_file_model,
                    'folder_id'=>$folder_id,
            ...
            if($fileUploaded){
                ...
                $model_file = new File();
                $model_file->name =  $fileUploaded->name;
                if(empty($model_file->description)) {$model_file->description = $uploadForm->file->name;}
                $model_file->file = 'ajax';
                $model_file->folder_id = $uploadForm->folder_id == null ? ($fileUploaded->type == 'img' ? 2 : ($fileUploaded->type == 'vid' ? 3 : 4)) : $uploadForm->folder_id;
                $model_file->path = $fileUploaded->path;
                $model_file->url= $fileUploaded->url;
                $model_file->type= $fileUploaded->type;
                $model_file->duration= $fileUploaded->duration;
                $model_file->pathThumb= $fileUploaded->pathThumb;
                $model_file->urlThumb= $fileUploaded->urlThumb;
                $model_file->extension = $fileUploaded->extension;
                $model_file->size = $fileUploaded->size;
                $model_file->preview = $fileUploaded->preview;
                $model_file->group_id = $this::getUserGroups()[0];
                
                if($uploadForm->save_file_model){
                    $save = $model_file->save();
                }else{
                    $save = true;
                }
                ...
                if ($fileUploaded && $save) {
                    ...
                    if($model !== null && $model_id !== null && $model_field !== null){

                        $modelClass = "app\\models\\{$model}";
                        $find = $modelClass::findOne($model_id);
                        $find->scenario = $find::SCENARIO_PICTURE;
                        $find->$model_field =  $model_file->id;
                        $model_save = $find->save();

                        if($model_field_id && $model_save){
                            $this->remove($model_field_id); 
                        }

                    }

                    return $this->asJson(['model' => $model_file,'model_save'=>$model_save]);
                }else{
                    return $this->asJson([
                        'result'=> $model_file->getErrors()
                    ]);
                }

            } 
        }
        return $this->asJson(['error' => true, 'info' => $uploadForm->getErrors()]);
    }
    ...
    public function actionUpload()
    {
        ...
        if ($this->request->isPost && $uploadForm->load($this->request->post())) {
            ...
            if($fileUploaded){
                ...
                $model = new File();
                $model->name =  $fileUploaded->name;
                if(empty($model->description)) {$model->description = $uploadForm->file->name;}
                $model->file = 'ajax';
                $model->folder_id = $uploadForm->folder_id == null ? ($fileUploaded->type == 'img' ? 2 : ($fileUploaded->type == 'vid' ? 3 : 4)) : $uploadForm->folder_id;
                $model->path = $fileUploaded->path;
                $model->url= $fileUploaded->url;
                $model->type= $fileUploaded->type;
                $model->duration= $fileUploaded->duration;
                $model->pathThumb= $fileUploaded->pathThumb;
                $model->urlThumb= $fileUploaded->urlThumb;
                $model->extension = $fileUploaded->extension;
                $model->size = $fileUploaded->size;
                $model->preview = $fileUploaded->preview;
                $model->group_id = $this::getUserGroups()[0];
