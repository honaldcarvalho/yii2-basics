<?php

/* @var $this \yii\web\View */
/* @var $content string */

use app\widgets\Alert;
use yii\base\View;
use yii\bootstrap5\Html;


\hail812\adminlte3\assets\AdminLteAsset::register($this);

$assetDir = Yii::$app->assetManager->getPublishedUrl('@vendor/almasaeed2010/adminlte/dist');
$publishedRes = Yii::$app->assetManager->publish('@vendor/hail812/yii2-adminlte3/src/web/js');

$this->registerCssFile('https://fonts.googleapis.com/css?family=Source+Sans+Pro:300,400,400i,700&display=fallback');
$this->registerCssFile('https://code.ionicframework.com/ionicons/2.0.1/css/ionicons.min.css');
$this->registerJsFile('@web/plugins/html2pdf/dist/html2pdf.bundle.min.js',['depends' => [\yii\web\JqueryAsset::class]]);

\hail812\adminlte3\assets\PluginAsset::register($this)->add(['fontawesome', 'icheck-bootstrap']);
$params = \weebz\yii2basics\modules\common\models\Params::findOne(1);

if(!empty($params->file_id)){
    $url = Yii::getAlias('@web').$params->file->urlThumb; 
    $logo_image = "<img alt='{$params->title}' width='150px' class='brand-image img-circle elevation-3' src='{$url}' style='opacity: .8' />";
}else{
    $logo_image = "<img src='/images/logo_weebz.png' width='100px' alt='{$params->title}' class='brand-image img-circle elevation-3' style='opacity: .8'>";
}

?>
<?php $this->beginPage() ?>
<!DOCTYPE html>
<html id="html">
<head>
    <meta charset="<?= Yii::$app->charset ?>">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <title><?=$params->title;?> | Log in</title>
    <!-- Tell the browser to be responsive to screen width -->
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <?php $this->registerCsrfMetaTags() ?>
    <?php $this->head(); ?>
    
    <!-- THEME -->
    <?= $this->registerCssFile("@web/css/themes.css"); ?>

</head>
<body>
<?php $this->beginBody() ?>
<div class="invoice p-3 mb-3">

    <div class="row">
        <div class="col-12">
            <h4>
                <div class="login-logo">
                    <?=$logo_image;?><br>
                    <b><?=$params->title;?></b> | <?=$this->title;?></a>
                </div>
                <small class="float-right">Date: <?=date('d/m/Y H:i')?></small>
            </h4>
        </div>

    </div>

    <div class="row">
        <div class="col-12 table-responsive">
            <?= $content ?>
        </div>
    </div>

</div>

<?php $this->endBody() ?>
</body>
</html>
<?php $this->endPage() ?>